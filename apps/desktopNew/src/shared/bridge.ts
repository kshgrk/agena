...
export type AgenaBridge = {
  ...
  saveProviderApiKey(
    id: string,
    input: { apiKey: string; env?: Record<string, string> },
  ): Promise<ProviderAuthSummary>;
  removeProviderAuth(id: string): Promise<ProviderAuthSummary>;
  startProviderOAuth(id: string): Promise<StartProviderOAuthResponse>;
  providerOAuthStatus(flowId: string): Promise<ProviderOAuthStatusResponse>;
  respondProviderOAuth(
    flowId: string,
    input: RespondProviderOAuthRequest,
