...
test("confirm maps to accepted", () => {
  assert.deepEqual(buildApprovalResponse("confirm", { optionId: null, text: "" }), {
    kind: "confirm",
    accepted: true,
  });
});
...
test("select requires a chosen option", () => {
  assert.equal(buildApprovalResponse("select", { optionId: null, text: "" }), null);
  assert.deepEqual(buildApprovalResponse("select", { optionId: "opt-2", text: "" }), {
    kind: "select",
    optionId: "opt-2",
  });
});
...
test("input and editor carry the text verbatim", () => {
  assert.deepEqual(buildApprovalResponse("input", { optionId: null, text: "hello" }), {
    kind: "input",
    text: "hello",
  });
  assert.deepEqual(
    buildApprovalResponse("editor", { optionId: "ignored", text: "line1\nline2" }),
...
test("initialText prefers defaultValue, editor falls back to subject.command", () => {
  ...
  assert.equal(initialText(req({ kind: "input", subject: { command: "c" } })), "");
...
test("approvalTitle: explicit title, then toolName, then generic", () => {
  assert.equal(approvalTitle(req({ title: "Delete branch?" })), "Delete branch?");
  assert.equal(approvalTitle(req({ subject: { toolName: "bash" } })), "Approve bash?");
...
test("subjectSummary prefers command, truncates long args json", () => {
  ...
  const long = subjectSummary(req({ subject: { args: { k: "x".repeat(200) } } }));
