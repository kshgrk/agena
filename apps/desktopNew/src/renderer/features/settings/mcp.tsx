...
> = {
  ...
  imported: { dot: "bg-success", label: "imported · not verified", tone: "muted" },
  ready: { dot: "bg-success", label: "ready · connects on use", tone: "muted" },
  needs_authorization: { dot: "bg-warn", label: "needs authorization", tone: "warn" },
  error: { dot: "bg-danger", label: "error", tone: "danger" },
};

const RESULT_TONE: Record<McpImportRunResult["mcps"][number]["status"], BadgeTone> = {
  imported: "success",
...
function ImportedMcpRow({
  ...
  return (
    <div className="p-3">
      ...
      {target ? (
        <div className="mt-0.5 truncate font-mono text-xs text-fg-muted" title={target}>
          {target}
        </div>
      ) : null}
