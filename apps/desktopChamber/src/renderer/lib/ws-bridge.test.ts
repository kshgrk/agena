...
test("wirePtyPort speaks the PtyPortMessage protocol over a fake socket", async () => {
  ...
  sock.onmessage?.({
    data: JSON.stringify({ type: "exit", exitCode: 3, signal: null }),
  });
...
test("createWsBridge: connect, subscribe, events+frames flow into one batch", async () => {
  ...
  const cmd = JSON.parse(sock.sent.at(-1)!) as {
    requestId: string;
    name: string;
  };
  ...
  sock.onmessage?.({
    data: JSON.stringify({ kind: "event", replayed: true, event: event(1) }),
  });
  sock.onmessage?.({
    data: JSON.stringify({ kind: "event", replayed: true, event: event(2) }),
  });
  sock.onmessage?.({
    data: JSON.stringify({
      kind: "sync",
  ...
  assert.deepEqual(batch.syncs, [
    { sessionId: "s1", branchId: "b1", upToSeq: 2 },
  ]);
  assert.equal(batch.frames.length, 1);
  assert.deepEqual(batch.frames[0]!.payload, {
    messageId: "m1",
    blockIndex: 0,
    delta: "hello",
  });
