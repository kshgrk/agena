...
export function createMockBridge(): AgenaBridge {
  ...
  return {
    ...
    async readCompactTranscript(
      ...
      const users = session.events.filter(
        (event) => event.type === "message.user.created",
      );
      const messageId = (event: AgenaEvent) =>
        String((event.payload as { messageId?: unknown }).messageId ?? "");
      const limit = opts?.limitTurns ?? 10;
      let end = users.length;
      if (opts?.beforeMessageId) {
        const index = users.findIndex(
          (event) => messageId(event) === opts.beforeMessageId,
        );
      ...
      if (opts?.aroundMessageId) {
        const index = users.findIndex(
          (event) => messageId(event) === opts.aroundMessageId,
        );
        if (index >= 0) {
          start = Math.max(0, index - Math.floor(limit / 2));
          end = Math.min(users.length, start + limit);
          start = Math.max(0, end - limit);
        }
      }
      const selected = users.slice(start, end);
      const turns = selected.map((user) => {
        const globalIndex = users.indexOf(user);
        const nextSeq = users[globalIndex + 1]?.seq ?? Number.POSITIVE_INFINITY;
        const entries: CompactTranscriptEntry[] = [];
        for (const event of session.events) {
          if (event.seq <= user.seq || event.seq >= nextSeq) continue;
          const base = {
            seq: event.seq,
            at: event.createdAt,
            source: event.source,
          ...
          if (event.type === "message.assistant.completed") {
            entries.push({
              ...base,
              kind: "assistant",
              messageId: String(payload.messageId ?? event.seq),
              content: (payload.content as ContentBlock[]) ?? [],
              status: "completed",
              ...(payload.model && typeof payload.model === "object"
                ? { model: payload.model as ModelRef }
                : {}),
            });
          } else if (event.type === "message.assistant.failed") {
            entries.push({
              ...base,
              kind: "assistant",
              messageId: String(payload.messageId ?? event.seq),
              ...
              status: "failed",
              error: (payload.error as { code: string; message: string }) ?? {
                code: "error",
                message: "failed",
              },
            });
          } else if (event.type === "message.assistant.aborted") {
            entries.push({
              ...base,
              kind: "assistant",
              messageId: String(payload.messageId ?? event.seq),
          ...
          } else if (event.type === "message.runtime.created") {
            entries.push({
              ...base,
              kind: "runtime",
              messageId: String(payload.messageId ?? event.seq),
              runtimeType:
                payload.runtimeType === "bash" ||
                payload.runtimeType === "branch-summary"
                  ? payload.runtimeType
                  : "custom",
              content: (payload.content as ContentBlock[]) ?? [],
            });
          } else if (event.type === "tool.call.started") {
            const toolCallId = String(payload.toolCallId ?? event.seq);
            const terminal = session.events.find(
              (candidate) =>
                candidate.seq > event.seq &&
                candidate.seq < nextSeq &&
                candidate.type.startsWith("tool.call.") &&
                candidate.type !== "tool.call.started" &&
                (candidate.payload as { toolCallId?: unknown }).toolCallId ===
                  toolCallId,
            );
            const terminalPayload = terminal?.payload as
              | Record<string, unknown>
              | undefined;
            const status = terminal?.type.endsWith(".completed")
              ? "completed"
              : terminal?.type.endsWith(".failed")
                ? "failed"
                : terminal?.type.endsWith(".aborted")
                  ? "aborted"
                  : terminal?.type.endsWith(".denied")
                    ? "denied"
                    : "running";
            entries.push({
              ...base,
              kind: "tool",
              toolCallId,
              messageId: String(payload.messageId ?? "assistant"),
              name: String(payload.name ?? "tool"),
              argsPreview: JSON.stringify(payload.args ?? "").slice(0, 320),
              status,
        ...
        const userPayload = user.payload as Record<string, unknown>;
        return {
          user: {
            seq: user.seq,
            at: user.createdAt,
            ...
            content: (userPayload.content as ContentBlock[]) ?? [],
          },
          entries,
        };
      });
      return {
        sessionId,
        branchId: session.summary.rootBranchId,
        upToSeq: session.summary.lastSeq,
        turns,
        hasOlder: start > 0,
        hasNewer: end < users.length,
      };
    },
    ...
    async getToolCallDetail(
      ...
      const started = session.events.find(
        (event) =>
          ...
          (event.payload as { toolCallId?: unknown }).toolCallId === toolCallId,
      );
      if (!started) throw new Error(`mock: unknown tool call ${toolCallId}`);
      ...
      return {
        ...
        messageId:
          typeof start.messageId === "string" ? start.messageId : undefined,
        name: String(start.name ?? "tool"),
        args: start.args,
        ...(terminalPayload?.result !== undefined
          ? { result: terminalPayload.result }
          : terminalPayload?.partialOutput !== undefined
            ? { result: terminalPayload.partialOutput }
            : {}),
        status: terminal?.type.endsWith(".completed")
          ? "ok"
          : terminal?.type.endsWith(".failed")
            ? "error"
            : terminal?.type.endsWith(".aborted")
        ...
        ...(terminal ? { endedSeq: terminal.seq } : {}),
